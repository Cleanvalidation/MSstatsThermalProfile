...
# Compare processing with Venn Diagrams
...
saveRDS(Benchmarks_TPPproc_p,"Zebra_TPPprocesed.RDS")
```
...
Compare TPP processing to MsstatsTMT Venn for each method
...
TPP_Zebra<-TPP_Zebra_results_F|>dplyr::filter(uniqueID %in% c("Q6NV46","Q68SP3","Q0H2G3"))
...
Stat_mod<-data.frame(Protein=NPARC_Zebra$id,NPARC_adjpval=formatC(NPARC_Zebra$pAdj, format = "e", digits = 2),TPP_adjpval=formatC(TPP_Zebra$p_adj_NPARC,format = "e", digits = 2),SCAM_adjpval=formatC(SCAM_Zebra$ATE_padjBH,format = "e", digits = 2),MSstats_adjpval=formatC(MSstat_Zebra$adj.pvalue,format = "e", digits = 2))
...
nudge_x = 3,
nudge_y=1,
force = 3)+ylab("log(adj.pvalue)")+xlab("ATE")+theme(text=element_text(size=18))
```
