...
```{r}
#WD<- "E:/internal_dataset/Excel/PSM"
...
summarised.proteins$ProteinLevelData<-summarised.proteins$ProteinLevelData|>dplyr::inner_join(set_temps(10,c(37.3, 40.6, 43.9, 47.2, 50.5, 53.8, 57.1, 60.4, 64, 67)))|>dplyr::inner_join(pd_annotation$annotation|>dplyr::select(Mixture,treatment)|>dplyr::distinct())
```

4.  Create a contrast matrix to measure the average treatment effect
    ...
    #choose three ranges around the midpoint
        ...
        temps_mid<-c("47.2","50.5","53.8")
        temps_ear<-c("40.6","43.9","47.2")
...
proteins<-summarised.proteins$ProteinLevelData |> dplyr::mutate(Channel=as.character(Channel),Mixture=as.character(Mixture))|>
  as.data.frame()
...
proteins1<-proteins|>dplyr::mutate(
...
```{r}
Benchmarks_MSstatsTMTproc<-list("TPP"=unique(TPPtr_human_results$Protein_ID[TPPtr_human_results$model_converged_treated_1 & TPPtr_human_results$model_converged_treated_2 & TPPtr_human_results$model_converged_vehicle_1 & TPPtr_human_results$model_converged_vehicle_2]),"NPARC"=unique(NPARC_test_emp$id),"SCAM"=unique(DIM_Human_SCAM$Accession),"MSstatsTMT"=DIM_late$ComparisonResult$Protein[is.na(DIM_late$ComparisonResult$issue)])
#remove shared accessions
...
```{r}
Benchmarks_MSstatsTMTproc_p<-list("TPP"=unique(TPPtr_human_results$Protein_ID[TPPtr_human_results$pVal_adj_vehicle_1_vs_treated_1<0.05 & TPPtr_human_results$pVal_adj_vehicle_2_vs_treated_2 < 0.05 & TPPtr_human_results$Protein_ID %in% TPPtr_filters]),"NPARC"=as.character(unique(NPARC_test_emp$id[NPARC_test_emp$pAdj<0.05&as.character(NPARC_test_emp$id) %in% TPPtr_filters])),"SCAM"=unique(DIM_Human_SCAM$Accession[DIM_Human_SCAM$ATE_padjBH<0.05]),"MSstatsTMT"=unique(DIM_late$ComparisonResult$Protein[DIM_late$ComparisonResult$adj.pvalue<0.05]))
#remove shared accessions
...
saveRDS(Human_MSstatsTMTprocesed,"Human_MSstatsTMTprocesed.RDS")
```
...
```{r}
png(filename = "Venn_Humandata_Tm_MSstat.png",
...
Stat_mod<-data.frame(Protein=MSstat_human$Protein,NPARC_adjpval=formatC(NPARC_human$pAdj, format = "e", digits = 2),TPP_adjpval=formatC(TPP_human$p_adj_NPARC,format = "e", digits = 2),SCAM_adjpval=formatC(SCAM_human$ATE_padjBH,format = "e", digits = 2),MSstats_adjpval=formatC(MSstat_human$adj.pvalue,format = "e", digits = 2))
...
```{r}
targets<-proteins1|>as.data.frame()|>dplyr::filter(Protein%in% c("P36507","Q02750"))|>dplyr::mutate(Accession=Protein)|>dplyr::inner_join(Stat_mod)|>dplyr::group_by(Accession)|>dplyr::group_split()
...
```{r}
targets<-proteins|>as.data.frame()|>dplyr::filter(Protein%in% c("P36507","Q02750"))|>dplyr::mutate(Accession=Protein)|>dplyr::inner_join(Stat_mod)|>dplyr::group_by(Accession)|>dplyr::group_split()
...
MSstatsTMT_proc<-lapply(Human_MSstatsTMTprocesed
                        ,function(x) as.character(x)|>na.omit())

TPP_proc<-lapply(Human_TPPprocesed,function(x) as.character(x)|>na.omit())
...
results<-data.frame(processing=rep("MSstatsTMT",3),"stat. model"=rep("MSstatsTMT",3),configuration=c("late","mid","early"),temperatures=stringr::str_c(temps_ear,temps_mid,temps_late,sep=","),"p-adj lt 0.05"=c(length(na.omit(unique(DIM_late$ComparisonResult$Protein[DIM_late$ComparisonResult$adj.pvalue<0.05]))),
                                                                                                                                                            length(unique(na.omit(ATE_mid$ComparisonResult$Protein[ATE_mid$ComparisonResult$adj.pvalue<0.05]))),
                                                                                                                                                            length(unique(na.omit(ATE_early$ComparisonResult$Protein[ATE_early$ComparisonResult$adj.pvalue<0.05])))),
                    targets=c(sum(DIM_late$ComparisonResult$adj.pvalue<0.05&DIM_late$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750")),
                              sum(ATE_mid$ComparisonResult$adj.pvalue<0.05&ATE_mid$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750")),sum(ATE_early$ComparisonResult$adj.pvalue<0.05&ATE_early$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750"))))
                                                                                                         results     
```
...
results_filtered<-data.frame(processing=rep("MSstatsTMT",3),"stat. model"=rep("MSstatsTMT",3),configuration=c("late","mid","early"),temperatures=stringr::str_c(temps_ear,temps_mid,temps_late,sep=","),"p-adj lt 0.05"=c(length(na.omit(unique(DIM_later$ComparisonResult$Protein[DIM_later$ComparisonResult$adj.pvalue<0.05]))),
                                                                                                                                                            length(unique(na.omit(ATE_middle$ComparisonResult$Protein[ATE_middle$ComparisonResult$adj.pvalue<0.05]))),
                                                                                                                                                            length(unique(na.omit(ATE_early$ComparisonResult$Protein[ATE_early$ComparisonResult$adj.pvalue<0.05])))),
                    targets=c(sum(DIM_later$ComparisonResult$adj.pvalue<0.05&DIM_later$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750")),
                              sum(ATE_middle$ComparisonResult$adj.pvalue<0.05&ATE_middle$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750")),sum(ATE_early$ComparisonResult$adj.pvalue<0.05&ATE_early$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750"))))
                                                                                                         results_filtered     
```
...
f$diffexpressed <- "Not Shifted"
    ...
    f$targets <-ifelse(f$Protein %in%"P36507","P36507",ifelse(f$Protein %in% "Q02750","Q02750",NA))
...
f$diffexpressed <- "Not Shifted"
    ...
    f$targets<-ifelse(f$Accession %in%"P36507","P36507",ifelse(f$Accession %in% "Q02750","Q02750",NA))
    f$delabel[f$Accession %in%c("P36507","Q02750","P40763","Q9Y2Q5","P31785")] <- as.character(f$Accession[f$Accession %in%c("P36507","Q02750","P40763","Q9Y2Q5","P31785")])
    flevels<-data.frame(colors=c("#762a83","#b35806","#1b7837"),
                        labels=c("Shifted adj. pVal < 0.05","Not Shifted","Shifted adj. pVal < 0.01"))
    flevels<-flevels %>% dplyr::filter(labels %in% unique(f$diffexpressed))
