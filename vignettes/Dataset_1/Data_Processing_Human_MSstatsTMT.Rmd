...
```
NPARC presents no statistically significant hits because we omitted shared proteins in our study and the other two proteins are not passing TPP filtering criteria
```{r}
NPARC_test_emp$id[which(NPARC_test_emp$pAdj<0.05)]
```
Plot the bar charts
...
```{r}
png(filename = "Venn_Humandata_MSstat.png",
...
results<-data.frame(processing=rep("MSstatsTMT",3),"stat. model"=rep("MSstatsTMT",3),configuration=c("late","mid","early"),temperatures=stringr::str_c(temps_ear,temps_mid,temps_late,sep=","),"p-adj lt 0.05"=c(length(na.omit(unique(DIM_late$ComparisonResult$Protein[DIM_late$ComparisonResult$adj.pvalue<0.05]))),
                    ...
                    targets=c(sum(DIM_late$ComparisonResult$adj.pvalue<0.05&DIM_late$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750")),
                              sum(DIM_mid$ComparisonResult$adj.pvalue<0.05&DIM_mid$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750")),sum(DIM_ear$ComparisonResult$adj.pvalue<0.05&DIM_ear$ComparisonResult$Protein%in%c("P36507",                                                                              "Q02750"))))
                                                                                                         results     
```
