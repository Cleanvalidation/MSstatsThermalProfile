...
path = getwd()
bHuman
ggplot2::ggsave(path = path, width = 24, height = 6.5, device='tiff', dpi=600, filename = "Bar_chartslate_MSstatsTMT.tiff")
...
#M. First calculate the total number of proteins from PD

```{r}
Total_prot<-list("TPP sigmoid"=unique(df$Master.Protein.Accessions),"NPARC"=unique(df$Master.Protein.Accessions),"SCAM"=unique(df$Master.Protein.Accessions),"MSstatsTMT"=unique(df$Master.Protein.Accessions))
#remove shared accessions
Total_prot<-lapply(Total_prot,function(x) x[!stringr::str_detect(x,";")])
#remove missing values
Total_prot<-lapply(Total_prot,function(x) as.character(x)|>na.omit())
...
Benchmarks_MSstatsTMTproc_p<-list("TPP sigmoid"=unique(TPPtr_human_results$Protein_ID[which(!is.na(TPPtr_human_results$pVal_adj_vehicle_1_vs_treated_1<0.05&TPPtr_human_results$pVal_adj_vehicle_2_vs_treated_2<0.05&TPPtr_human_results$R_sq_treated_1>0.8|TPPtr_human_results$R_sq_treated_2>0.8&sign(TPPtr_human_results$diff_meltP_vehicle_1_vs_treated_1)==sign(TPPtr_human_results$diff_meltP_vehicle_2_vs_treated_2)&TPPtr_human_results$passed_filter_vehicle_1_vs_treated_1&TPPtr_human_results$passed_filter_vehicle_2_vs_treated_2))]),"NPARC"=as.character(unique(NPARC_test_emp$id[which(NPARC_test_emp$pAdj<0.05&NPARC_test_emp$id %in% TPPtr_filters)])),"SCAM"=unique(DIM_Human_SCAM$Accession[DIM_Human_SCAM$ATE_padjBH<0.05]),"MSstatsTMT"=unique(DIM_late$ComparisonResult$Protein[DIM_late$ComparisonResult$adj.pvalue<0.05]))
#remove shared accessions
Benchmarks_MSstatsTMTproc_p<-lapply(Benchmarks_MSstatsTMTproc_p,function(x) x[!stringr::str_detect(x,";")])
#remove missing values
Benchmarks_MSstatsTMTproc_p<-lapply(Benchmarks_MSstatsTMTproc_p,function(x) as.character(x)|>na.omit())
...
names(Benchmarks_MSstatsTMTproc_p)<-c("TPP sigmoid stat. model","NPARC stat. model","SCAM stat. model","MSstatsTMT stat. model")
Dataset_1_psm_results<-Benchmarks_MSstatsTMTproc_p
saveRDS(Dataset_1_psm_results,"Dataset_1_psm_results_sigmoid.RDS")
```
Append MSstatsTMT fitted models to the data
...
#N.   Plot the curves Figures S-9 and S-11, panels E-H and save data
...
nam<-as.character(unique(dplyr::bind_rows(targets)$Protein))
target_curves<-lapply(fitted_MSstat,function(x){plotMethods(x,temps=c("53.8","57.1","60.4"),labels=c("E","F","G","H"),processing="MSstatsTMT", fit="Sigmoid")
})
names(target_curves)<-nam
#get path
...
```
#O.First calculate the total number of proteins from PD
...
### Panel C row Statistical model fit convergence

```{r}
Benchmarks_MSstatsTMTproc<-list("TPP"=unique(TPP_human_results_F$uniqueID[!is.na(TPP_human_results_F$p_adj_NPARC)]),"NPARC"=unique(NPARC_test_emp$id),"SCAM"=unique(DIM_Human_SCAM$Accession),"MSstatsTMT"=DIM_mid$ComparisonResult$Protein[is.na(DIM_mid$ComparisonResult$issue)])
#remove shared accessions
Benchmarks_MSstatsTMTproc<-lapply(Benchmarks_MSstatsTMTproc,function(x) x[!stringr::str_detect(x,";")])
#remove missing values
Benchmarks_MSstatsTMTproc<-lapply(Benchmarks_MSstatsTMTproc,function(x) x[!is.na(x)])
...
names(Benchmarks_MSstatsTMTproc_p)<-c("TPP stat. model","NPARC stat. model","SCAM stat. model","MSstatsTMT stat. model")
Dataset_1_psm_results<-Benchmarks_MSstatsTMTproc_p
saveRDS(Dataset_1_psm_results,"Dataset_1_psm_resultsmid.RDS")
```
#P. Figure S-26, Panel C bar charts
Plot the bar charts
