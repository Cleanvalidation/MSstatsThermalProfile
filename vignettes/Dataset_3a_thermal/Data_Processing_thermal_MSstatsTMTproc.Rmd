...
# Fit models using filtered data without the variance contribution of the 7 remaining channels
...
# Venn Diagrams comparing overlaps between publication and our work

```{r}

#Published work
OnePotTPP_2D_hits<-kinase_df$Accession[stringr::str_detect(kinase_df$`DetectedOnePotTPP?`,"Y")]

SPROX_2D_hits<-kinase_df$Accession[stringr::str_detect(kinase_df$`DetectedOnePotSPROX?`,"Y")]

OnePot_SPROX_hits<-kinase_df$Accession[stringr::str_detect(kinase_df$`OnePotSPROXHits?`,"Y")]

TPP_hits<-kinase_df$Accession[stringr::str_detect(kinase_df$`OnePotTPPHits?`,"Y")]

#This work
NPARC_hits<-Benchmarks_MSstatsTMTproc_p$NPARC
TPPspline_hits<-Benchmarks_MSstatsTMTproc_p$TPP
MSstat_hits<-Benchmarks_MSstatsTMTproc_p$MSstatsTMT


Benchmarks_external<-list(#"OnePot 2D TPP"=OnePotTPP_hits,
                          #"SPROX OnePot"=SPROX_hits,
                          "TPP (Xu. et al)"=TPP_hits,
                         # "TPP (this work)"=TPPspline_hits,
                          "NPARC (this work)"=NPARC_hits)#,
                         # "MSstatsTMT"=MSstat_hits)

png(filename = "Venn_TPPpapervsThiswork_MSstatproc.png",
    width =600, height = 600, units = "px", pointsize = 12,
    res = 130,type ="cairo")  
...
# Thermal targets significant for protein-level hits

```{r}
GNS<-summarised.proteins$ProteinLevelData|>
  dplyr::filter(Protein=="P15586")|>
  dplyr::mutate(treatment=stringr::str_extract(treatment,"[[:lower:]]+"))|>
  dplyr::group_split(Condition)
GNS_renamed<-lapply(GNS,function(x) {
...
ggplot(GNS_renamed,mapping=aes(x=temperature,y=Abundance,color=treatment))+geom_point()+ylab(expression(log[2]~Abundance))


```

```{r}
STK3<-summarised.proteins$ProteinLevelData|>
  dplyr::filter(Protein=="Q13188")|>
  dplyr::mutate(treatment=stringr::str_extract(treatment,"[[:lower:]]+"))|>
  dplyr::group_split(Condition)
STK3_renamed<-lapply(STK3,function(x) {
  x$Condition<-as.character(x$Condition)
  x$Condition<-ifelse(x$Condition=="DMSO","00xDMSO",x$Condition)
  x$Condition<-ifelse(x$Condition=="1xstauro","01xstauro",x$Condition)
  x$Condition<-ifelse(x$Condition=="5xstauro","05xstauro",x$Condition)
  ...
  x$Condition<-ifelse(x$Condition=="25xstauro","25xstauro",x$Condition)
                            return(x)})|>dplyr::bind_rows()
ggplot(STK3_renamed,mapping=aes(x=temperature,y=Abundance,color=treatment))+geom_point()+ylab(expression(log[2]~Abundance))


```

```{r}
CHEK1<-summarised.proteins$ProteinLevelData|>
  dplyr::filter(Protein=="O14757")|>
  dplyr::mutate(treatment=stringr::str_extract(treatment,"[[:lower:]]+"))|>
  dplyr::group_split(Condition)
CHEK1_renamed<-lapply(CHEK1,function(x) {
  ...
  x$Condition<-ifelse(x$Condition=="25xstauro","25xstauro",x$Condition)
                            return(x)})|>dplyr::bind_rows()
ggplot(CHEK1_renamed,mapping=aes(x=temperature,y=Abundance,color=treatment))+geom_point()+ylab(expression(log[2]~Abundance))
