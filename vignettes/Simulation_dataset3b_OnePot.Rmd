---
title: "Simulation_dataset3b_OnePot"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Simulation_dataset3b_OnePot}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
library(MSstatsThermalProfiler)
library(ggplot2)
library(tidyr)
library(MSstats)
library(MSstatsTMT)
```

#A. Load in MSstatsTMT processed data

```{r}
#read summarised protein output from MSstatsTMT
summarised.proteins<- readRDS("~/MSstatsThermalProfiler/vignettes/Dataset_3a-b_simulation_data/summarised.proteinsD1.RDS")
summarised.proteinsNS<- readRDS("~/MSstatsThermalProfiler/vignettes/Dataset_3a-b_simulation_data/summarised.proteinsZebra.RDS")
```

#B.Figure 6, panel D design: 2 TMT 10-plexes, 10 biological replicates and 2 temperatures 
###Print out ICC distributions and simulate a strong interactor for a design with two temperatures, 2 plexes, 10 replicates

```{r}
#get variance components
Dataset_1_ICC<-msstats_icc(summarised.proteins,temps=unique(summarised.proteins$ProteinLevelData$temperature))
#generate simulations
 OnePot_2plex_10repsstrong<-benchmark_shifter_sim_2plex(Dataset_1_ICC,templateProtein="P36507",n_sims=1000,t_range=seq(5,6),design="OnePot",shifter="strong")
Histograms_2temp_strong_onePot_10reps<-plot_benchmarks_MSstatsTMT_reps( OnePot_2plex_10repsstrong,design="OnePot",shifter="strong",t_range=seq(5,6),n_replicates_per_plex =5)
```
###Print out ICC distributions and simulate a weak interactor for a design with two temperatures, 2 plexes, 5 replicates per plex  by Condition

```{r}
#generate simulations
 OnePot_2plex_10repsweak<-benchmark_shifter_sim_2plex(Dataset_1_ICC,templateProtein="O00422",n_sims=1000,t_range=seq(5,6),design="OnePot",shifter="weak")
Histograms_2temp_strong_onePot_10reps<-plot_benchmarks_MSstatsTMT_reps( OnePot_2plex_10repsweak,design="OnePot",shifter="weak",t_range=seq(5,6),n_replicates_per_plex =5)
```
##Print out ICC distributions and simulate a non-interactor for a design with two temperatures, 2 plexes, 5 replicates per plex by Condition

```{r}
#get variance components
Dataset_2_ICC<-msstats_icc(summarised.proteinsNS,temps=unique(summarised.proteinsNS$ProteinLevelData$temperature))
#generate simulations

 OnePot_2plex_10repsnon<-benchmark_nonshifter_sim_2plex(Dataset_2_ICC,templateProtein="Q6NV46",n_sims=1000,t_range=seq(5,6),design="OnePot")
Histograms_2temp_non_onePot_10reps<-plot_benchmarks_MSstatsTMT_reps(  OnePot_2plex_10repsnon,design="OnePot",shifter="non",t_range=seq(5,6),n_replicates_per_plex =5)
```
#C. Figure 6, panel E: for a design with 1 TMT 10-plex, 1 biological replicate per mixture, 2 temperatures per plex and 2 temperatures as contrast
```{r}
Two_temps_tworeps_cond_10reps_weak<-result_weak_2|>dplyr::group_by(Protein,Condition,temperature)|>dplyr::group_split()|>lapply(function(x)x|>dplyr::mutate(replicate=seq(1,10)))|>dplyr::bind_rows()

Two_temps_tworeps_cond_10reps_non<-result_non_2|>dplyr::group_by(Protein,Condition,temperature)|>dplyr::group_split()|>lapply(function(x)x|>dplyr::mutate(replicate=seq(1,10)))|>dplyr::bind_rows()
```
Pooled thermal designs: 2 plexes, 10 replicates per condition, two pooled temperatures per replicate

```{r}
Histograms_2temp_strong_onePot_10reps<-plot_benchmarks_MSstatsTMT_reps(Two_temps_tworeps_cond_10reps_strong,design="OnePot",shifter="strong",n_replicates_per_plex=10)
```


```{r}
Histograms_2temp_weak_onePot_10reps<-plot_benchmarks_MSstatsTMT_reps(Two_temps_tworeps_cond_10reps_weak,design="OnePot",shifter="weak",n_replicates=10)
  
Histograms_2temp_non_onePot_10reps<-plot_benchmarks_MSstatsTMT_reps(Two_temps_tworeps_cond_10reps_non,design="onePot",shifter="non",n_replicates=10)
```
