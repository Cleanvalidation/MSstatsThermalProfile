...
MSstatsTMT stat. mod."=Thermal_lateMSstatsTMTprocesed$`MSstatsTMT temp.`),list(
  ...
  "3b: MSstatsTMT proc. 
  OnePotMSstatsTMT stat. mod."=MSstat_hits))
figure8D<-ggvenn::ggvenn(data=Benchmarks_external,fill_color =c("#2c7fb8","#7b057b","#1d547a"),set_name_size=5,text_size=8,stroke_size=1.5,fill_alpha = 0.6,show_percentage = FALSE)+theme(text=element_text(size=20))+coord_cartesian(ylim = c(-2,2),xlim = c(-4,4))
...
summarised.proteins<-lapply(summarised.proteins,function(x) {
  ...
  return(x)})
P36507<-MSstatsTMT::dataProcessPlotsTMT(summarised.proteins,type="PROFILEPLOT",which.Protein="P36507",address=FALSE)

P36507<-P36507+theme(text=element_text(size=40))

ggsave(filename = "OnePot_P36507profile.png",
    width =12, height = 12, units = "in", pointsize = 40,
    dpi = 600,type ="cairo")  
...
Figure8A_3aTPPprocesed <- readRDS("~/MSstatsThermalProfiler/vignettes/Dataset_3b_OnePot/Figure8A_3aTPPprocesed.RDS")


Figure8A_3aMSstatsTMTprocesed <- readRDS("~/MSstatsThermalProfiler/vignettes/Dataset_3b_OnePot/Figure8A_3aMSstatsTMTprocesed.RDS")

Benchmarks_external<-append(list("3a: TPP proc.
MSstatsTMT stat. mod."=Figure8A_3aTPPprocesed$`MSstatsTMT temp.`),list(
  "Reference Set"=na.omit(kinases$Accession),
  "3a: MSstatsTMT proc.
 MSstatsTMT stat. mod."=Figure8A_3aMSstatsTMTprocesed$`MSstatsTMT temp.`))
library(ggvenn)
Figure8A<-ggvenn::ggvenn(data=Benchmarks_external,fill_color =c("#1d547a","#7b057b","#1d547a"),set_name_size=5,text_size=8,stroke_size=1.5,fill_alpha = 0.6,show_percentage = FALSE)+theme(text=element_text(size=20))+coord_cartesian(ylim = c(-2,2),xlim = c(-4,4))
png(filename = "Fig8A.png",
...
library(ggvenn)
Figure8D<-ggvenn::ggvenn(data=Benchmarks_external,fill_color =c("#1d547a","#7b057b","#1d547a"),set_name_size=5,text_size=8,stroke_size=1.5,fill_alpha = 0.6,show_percentage = FALSE)+theme(text=element_text(size=20))+coord_cartesian(ylim = c(-2,2),xlim = c(-4,4))
