...
Converter_MSstats_Format_to_TPP<-function(x,CARRIER=FALSE){
  ...
  if(any(names(x)=="TechRepMixture")){
    x$TechRepMixture<-as.character(x$TechRepMixture)
    if(length(unique(x$Subject))==4){
      reps<-x|>dplyr::select(Subject,TechRepMixture,Condition)|>dplyr::distinct()|>dplyr::group_by(Condition)|>dplyr::mutate(TechRepMixture=(rep(c(1,2),1)))|>dplyr::ungroup()
      x<-x|>dplyr::select(-TechRepMixture)|>dplyr::inner_join(reps)
    }
  ...
  if(any(names(x)=="treatment")&length(unique(x$Condition))==2){
    x$Condition<-ifelse(stringr::str_detect(
      stringr::str_to_lower(x$treatment),
      "vehicle"),"Vehicle","Treatment")
    x$Experiment<-paste0(x$Condition,"_",x$TechRepMixture)
    x$Mixture<-x$Experiment
  }
