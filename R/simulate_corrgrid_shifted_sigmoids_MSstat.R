simulate_corrgrid_shifted_sigmoids_MSstat = function(runs=1000,n=4,error_sd=0.05,subject_sd=seq(0,0.15,0.025),half_temp_grid=FALSE){
  ...
  for(i in 1:length(subject_sd)){
    subSD = subject_sd[i]
    sim = simulate_multiple_shifted_sigmoids_MSstat(runs=runs,n=n,error_sd=error_sd,subject_sd=subSD,half_temp_grid=half_temp_grid)
