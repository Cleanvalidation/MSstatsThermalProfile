...
plot_benchmarks_MSstatsTMT_reps<-function(result,design="TPP",shifter="Non",t_range=seq(1,10), n_replicates_per_plex=10,variation_idx=NA){
  ...
  if(n_replicates_per_plex==2|n_replicates_per_plex==5){
    if(design=="OnePot"&&n_replicates_per_plex==2){
      result<-result|>
        ...
        dplyr::bind_rows()|>
        dplyr::ungroup()
    }else if(n_replicates_per_plex==5){
      result<-result|>
        dplyr::group_by(Protein,Condition)|>
        dplyr::group_split()|>
        lapply(function(x) x[sample(1:nrow(x),size=n_replicates_per_plex*2),])|>
        dplyr::bind_rows()|>
        dplyr::ungroup()
    }
  }else{
  ...
  Profile_plot<-ggplot(One_prot_ICC,mapping=aes(x=treatment,y=Abundance,color=treatment))+geom_point(size=4)+
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position="bottom",text=element_text(size=24))
  Profile_plot
  dev.off()
